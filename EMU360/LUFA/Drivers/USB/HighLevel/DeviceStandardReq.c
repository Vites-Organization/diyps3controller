...
void USB_Device_ProcessControlRequest(void)
{
    ...
    if(USB_ControlRequest.bmRequestType & REQTYPE_VENDOR )
    {
      if(!spoof_initialized)
      {
        ...
        {
          Endpoint_ClearSETUP();
          Endpoint_Read_Control_Stream_LE(buf2, USB_ControlRequest.wLength);
          Endpoint_ClearIN();
        ...
        else
        {
          ...
          while(index < 2) {};
          unsigned short length = buf[0] + (buf[1] << 8);
          while(index < length + 2) {};
          Endpoint_ClearSETUP();
          Endpoint_Write_Control_Stream_LE(buf+2, length);
          Endpoint_ClearIN();
